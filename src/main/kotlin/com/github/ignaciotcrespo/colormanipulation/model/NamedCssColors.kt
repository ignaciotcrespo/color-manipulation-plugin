...
object NamedCssColors {
    val nameToHex: Map<String, String> = mapOf(
        "aliceblue" to "#F0F8FF",
    ...
    val hexToName: Map<String, String> = run {
        val map = mutableMapOf<String, String>()
        for ((name, hex) in nameToHex) {
            val key = hex.uppercase()
            // Prefer shorter names and skip grey variants (prefer gray)
            if (key !in map || name.length < map[key]!!.length) {
                map[key] = name
            }
        }
        map
    ...
    fun isNamedColor(text: String): Boolean = nameToHex.containsKey(text.lowercase().trim())

    fun getColor(name: String): UnifiedColor? {
        val hex = nameToHex[name.lowercase().trim()] ?: return null
        return ColorConverter.parse(hex)?.first
    }

    fun getName(color: UnifiedColor): String? {
    ...
    fun allNames(): Set<String> = nameToHex.keys

    fun findClosest(color: UnifiedColor): Pair<String, UnifiedColor> {
        return nameToHex.entries.map { (name, hex) ->
            val c = ColorConverter.parse(hex)?.first ?: return@map null
            val dr = color.r - c.r; val dg = color.g - c.g; val db = color.b - c.b
            Triple(name, c, dr * dr + dg * dg + db * db)
        }.filterNotNull().minBy { it.third }.let { it.first to it.second }
    }
}
